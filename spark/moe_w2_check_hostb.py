...
for e in range(E):
    ...
    _hostb = np.ascontiguousarray(pack_fragment_major(codes).numpy()); d_b = ctypes.c_uint64(_hostb.ctypes.data); _keep.append(_hostb)
    _hostbs = np.ascontiguousarray(pack_scales(sexp).numpy()); d_bs = ctypes.c_uint64(_hostbs.ctypes.data); _keep.append(_hostbs)
    d_c = cu.alloc(M * N * 2)
    d_cs.append(d_c)
