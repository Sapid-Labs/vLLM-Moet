...
def _dense_kv_rows(attn: nn.Module) -> torch.Tensor:
    """The K and V rows of a layer's qkv_proj, in the activation dtype.

    Called from `_build_fused_kv_buffers`, i.e. after `load_weights` but before
    `process_weights_after_loading`, so a quantized checkpoint is still in its
    on-disk layout here. For compressed-tensors that is `weight_packed`
    [out, in // pack_factor] int32 packed along the input dim, plus
    `weight_scale` [out, in // group_size]; the output dim is dim 0 in both, so
    the `[q_size:]` row slice is taken before unpacking to keep it cheap.
    """
    qkv = attn.qkv_proj
    weight = getattr(qkv, "weight", None)
    if weight is not None and weight.dtype not in (torch.int32, torch.uint8):
    ...
    if packed is None or scale is None:
        raise NotImplementedError(
            "DFlash fused context-KV precompute cannot recover dense K/V rows "
            f"from qkv_proj of type {type(qkv).__name__} with params "
            f"{sorted(n for n, _ in qkv.named_parameters())}. Add a branch here "
            "for this quantization format."
        )

    packed = packed.data[attn.q_size :]
    scale = scale.data[attn.q_size :]
    out_features = packed.shape[0]
    pack_factor = 32 // 4
    in_features = packed.shape[1] * pack_factor
    # Guard the ordering assumption: marlin's `process_weights_after_loading`
    # repacks `weight_packed` into a tile layout whose shape no longer decodes
    # this way, and unpacking it would silently yield garbage rather than fail.
    if in_features != attn.hidden_size or out_features != 2 * attn.kv_size:
        raise RuntimeError(
            "qkv_proj.weight_packed is not in compressed-tensors layout "
            f"(decoded [{out_features}, {in_features}], expected "
            f"[{2 * attn.kv_size}, {attn.hidden_size}]); _build_fused_kv_buffers "
            "must run before process_weights_after_loading."
        )
    group_size = in_features // scale.shape[1]

    shifts = torch.arange(pack_factor, device=packed.device, dtype=torch.int32) * 4
    # uint4b8: stored unsigned in [0, 15] with a symmetric bias of 8.
    q = ((packed.unsqueeze(-1) >> shifts) & 0xF).to(torch.int32) - 8
    q = q.reshape(out_features, in_features)
    deq = q.reshape(out_features, -1, group_size) * scale.unsqueeze(-1).to(torch.float32)
    return deq.reshape(out_features, in_features).to(scale.dtype)
...
class DFlashQwen3Attention(nn.Module):
    ...
    def forward(
        ...
        # NOTE: must go through the module (not F.linear on .weight): under a
        # quantized checkpoint `.weight` is either absent (marlin stores
        # `weight_packed`) or transposed (fp8 PTPC), so F.linear would raise or
        # silently return garbage. QKVParallelLinear is column-parallel with
        # gather_output=False, so this is the same math plus quant dispatch.
...
class DFlashQwen3Model(nn.Module):
    ...
    def _build_fused_kv_buffers(self) -> None:
        ...
        # Always materialized in the activation dtype: the fused context-KV
        # precompute is one GEMM over the whole context, run once per cycle,
        # and it is not on the per-step critical path -- so it is not worth a
        # quantized kernel. Keeping it dense also means this buffer survives
        # `process_weights_after_loading` repacking the source layers.
