...
class GPUModelRunner(
    ...
    def sample_tokens(
        ...
        if not getattr(self, "_mtp_probed", False):
            self._mtp_probed = True
            try:
                import os as _os
                import socket as _sk
                from vllm.distributed.parallel_state import get_pp_group as _gpp
                g = _gpp()
                with open("/tmp/mtp_probe.log", "a") as _fh:
                    _fh.write(
                        f"host={_sk.gethostname()} pid={_os.getpid()} "
                        f"cls={type(self).__name__} "
                        f"rank_in_group={g.rank_in_group} rank={g.rank} "
                        f"ws={g.world_size} is_first={g.is_first_rank} "
                        f"is_last={g.is_last_rank} "
                        f"async={self.use_async_scheduling} "
                        f"bcast_pp={self.broadcast_pp_output} "
                        f"spec={self.num_spec_tokens} "
                        f"state_none={self.execute_model_state is None}\n")
            except Exception as e:
                with open("/tmp/mtp_probe_err.log", "a") as _fh:
                    _fh.write(repr(e) + "\n")
    ...
    def _mtp_trace(self, msg: str) -> None:
        from vllm.distributed.parallel_state import get_pp_group
        type(self)._MTP_TRACE_N += 1
        with open(f"/tmp/mtp_trace_rank{get_pp_group().rank}.log", "a") as fh:
            fh.write(f"{type(self)._MTP_TRACE_N:06d} {msg}\n")
    ...
    def _pp_broadcast_prev_sampled_token_ids(
        ...
        if not self._is_all_reqs_chunked_prefill():
            self._mtp_trace(f"S.samp n={sampled_token_ids.shape[0]} "
                            f"w={sampled_token_ids.shape[1]} "
                            f"sum={int(sampled_token_ids.sum())}")
            torch.distributed.broadcast(
                sampled_token_ids, src=pp.rank, group=pp.device_group
            )
        else:
            self._mtp_trace("S.samp SKIP")
    ...
    def _pp_receive_prev_sampled_token_ids_to_input_batch(self) -> None:
        ...
        if not self._is_all_reqs_chunked_prefill():
            ...
            self._mtp_trace(f"R.samp n={num_reqs} w={width} sum={int(recv.sum())}")
        else:
            self._mtp_trace("R.samp SKIP")
    ...
    def _pp_broadcast_draft_token_ids(self) -> None:
        ...
        if isinstance(draft, torch.Tensor) and draft.dim() == 2:
            ...
            out[:r, :c] = d[:r, :c]
        self._mtp_trace(f"S.draft n={num_reqs} w={width} sum={int(out.sum())}")
    ...
    def _pp_receive_draft_token_ids(self) -> None:
        ...
        assert not pp.is_last_rank
        if self._is_all_reqs_chunked_prefill():
            self._mtp_trace("R.draft SKIP")
            return
        num_reqs = self.input_batch.num_reqs
        width = self.num_spec_tokens
        recv = torch.empty((num_reqs, width), dtype=torch.int32, device=self.device)
        torch.distributed.broadcast(recv, src=pp.last_rank, group=pp.device_group)
        self._mtp_trace(f"R.draft n={num_reqs} w={width} sum={int(recv.sum())}")
