...
def update_dspark(config_dict: dict, pre_trained_config: dict) -> None:
    ...
    # This fork's v1/spec_decode base proposer reads the parallel-drafting mask
    # token and target layers from `dflash_config` (DSpark reuses DFlashProposer's
    # parallel-drafting machinery). Mirror the dflash layout here.
    pre_trained_config["dflash_config"] = {
        "mask_token_id": config_dict["mask_token_id"],
        "target_layer_ids": [i - 1 for i in aux_layer_ids],
        "causal": False,
